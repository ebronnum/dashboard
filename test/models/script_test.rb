...
class ScriptTest < ActiveSupport::TestCase
  ...
  test 'should not change Script or ScriptLevel ID when reseeding' do
    x = Script.setup([], [@script_file])
  ...
  test 'should not change script ID when changing script options' do
    x = Script.setup([], [@script_file])
    script_id = x[0][4].script_id
    parsed_csv = Script.parse_csv(`config/generate_scripts #{@script_file}`, "\t", Script::SCRIPT_MAP)
    parsed_csv[4]['level_num'] = "Level 1"
    # 'trophies' and 'hidden' options changed from custom level options in Script.setup
